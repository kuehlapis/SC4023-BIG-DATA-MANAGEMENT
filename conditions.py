...
def _matric_digits(matric_num: str) -> list[str]:
...
def towns_from_matric(matric_num: str) -> list[str]:
    """Return sorted unique towns mapped from digits in matric number."""
    digits = _matric_digits(matric_num)
    towns = {TOWN_BY_DIGIT[d] for d in digits}
    return sorted(towns)


def target_year_from_matric(matric_num: str) -> int:
    """
    Map the last digit in matric number to 2015..2024.
    If last digit is 5, year=2015; ...; 9=>2019; 0=>2020; ...; 4=>2024.
    """
    last_digit = int(_matric_digits(matric_num)[-1])
    return 2010 + last_digit if last_digit >= 5 else 2020 + last_digit


def start_month_from_matric(matric_num: str) -> int:
    """Use second-last digit as month, with 0 interpreted as October (10)."""
    second_last = int(_matric_digits(matric_num)[-2])
    return 10 if second_last == 0 else second_last


def build_time_window(year: int, start_month: int, x: int) -> list[tuple[int, int]]:
    """
